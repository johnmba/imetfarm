...
from cms_toolbar import ADMIN_MENU_IDENTIFIER, ADMINISTRATION_BREAK
...
class NewsToolbar(CMSToolbar):
    ...
    def populate(self):
        ...
        newsitem_menu = admin_menu.get_or_create_menu('news-menu', _('Products ...'), position=position)
        ...
        newsitem_menu.add_sideframe_item(_('Products Category List'), url=url)

        url = reverse('admin:news_newscategory_add')
        newsitem_menu.add_modal_item(_('Add New Products Category'), url=url)

        newsitem_menu.add_break()

        url = reverse('admin:news_newsitem_changelist')
        newsitem_menu.add_sideframe_item(_('Products Item List'), url=url)
